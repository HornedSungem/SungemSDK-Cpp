...
Result::Result(const std::string &cnn_type)
    : classification_result(nullptr), detection_result(nullptr) {
...
DetectionResultPtr Result::getDetectionResult() { return detection_result; }

void Result::parseYoloResult(const std::vector<float> &result,
        ...
        for (int j = 0; j < grid_width; j++) {
            for (int k = 0; k < bbox_num; k++) {
                ...
                std::vector<float> probs(result.begin() + index / 2 * class_num,
                                         result.begin() +
                                             index / 2 * class_num + class_num);
                float scale = result[prob_num + index];
                std::vector<float>::iterator max_iter =
                    std::max_element(std::begin(probs), std::end(probs));
                obj_in_bbox.item.probability = *max_iter * scale;

                if (obj_in_bbox.item.probability > prob_threshold) {
                    obj_in_bbox.item.category =
                        categories[std::distance(std::begin(probs), max_iter)];
                    int x_center =
                        ((result[prob_num + bbox_conf_num + index * 4] + j) /
                         7.0) *
                        img_width;
                    int y_center =
                        ((result[prob_num + bbox_conf_num + index * 4 + 1] +
                          i) /
                         7.0) *
                        img_height;
                    obj_in_bbox.bbox.width =
                        (result[prob_num + bbox_conf_num + index * 4 + 2]) *
                        (result[prob_num + bbox_conf_num + index * 4 + 2]) *
                        img_width;
                    obj_in_bbox.bbox.height =
                        (result[prob_num + bbox_conf_num + index * 4 + 3]) *
                        (result[prob_num + bbox_conf_num + index * 4 + 3]) *
                        img_height;
                    obj_in_bbox.bbox.x =
                        (x_center - 0.5 * obj_in_bbox.bbox.width) < 0
                            ? 0
                            : (x_center - 0.5 * obj_in_bbox.bbox.width);
                    obj_in_bbox.bbox.y =
                        (y_center - 0.5 * obj_in_bbox.bbox.height) < 0
                            ? 0
                            : (y_center - 0.5 * obj_in_bbox.bbox.height);
...
void Result::parseSSDResult(const std::vector<float> &result,
                            const std::vector<std::string> categories,
                            int img_width, int img_height) {
    ...
    for (int i = 0; i < num_detection; i++) {
        ...
        ItemInBBox obj_in_bbox;
        if (std::isnan(category_id) || std::isnan(probability) ||
            std::isnan(xmin) || xmin < 0 || xmin > img_width ||
            std::isnan(ymin) || ymin < 0 || ymin > img_height ||
            std::isnan(xmax) || xmax < 0 || xmax > img_width ||
            std::isnan(ymax) || ymax < 0 || ymax > img_height) {
            continue;
        } else {
...
void Result::NMS(ItemInBBoxArrayPtr objs_in_bboxes) {
    ...
    for (auto iter1 = objs_in_bboxes->begin(); iter1 != objs_in_bboxes->end();
         iter1++) {
...
float Result::IOU(ItemInBBox box1, ItemInBBox box2) {
    int xmax = (box1.bbox.x + box1.bbox.width < box2.bbox.x + box2.bbox.width)
                   ? box1.bbox.x + box1.bbox.width
                   : box2.bbox.x + box2.bbox.width;
    ...
    int ymax = (box1.bbox.y + box1.bbox.height < box2.bbox.y + box2.bbox.height)
                   ? box1.bbox.y + box1.bbox.height
                   : box2.bbox.y + box2.bbox.height;
    ...
    return inter_area * 1.0 /
           (box1.bbox.width * box1.bbox.height +
            box2.bbox.width * box2.bbox.height - inter_area);
}
